...
<html xmlns="http://www.w3.org/1999/xhtml">	
	<head>
		...
		<script type="text/javascript">
			...
			function formSubmit() {
				...
				var vcDeptId = $('#vcDeptId').val();
				var url = '${contextPath}/department/deptinfo/ajaxCheckDeptNameUnique.action';
				$.post(url, {'pmBscDeptInfo.vcDeptName': vcDeptName, 'pmBscDeptInfo.pid': vcDeptId}, function(jsonObj) {
					if(jsonObj.timeout) {
	...
	<body>
		<div class="iner_title_name"><strong>机构信息管理>>机构信息管理>>修改</strong></div>
		<div class="iner_title_sub_03">
			<a href="javascript:doCancel();" class="return" >返回</a>
		</div>
		<@s.set name="formUrl" value="'/department/deptinfo/editDepType.action'" />
		<#include "form.ftl" />
	</body>
</html>		
