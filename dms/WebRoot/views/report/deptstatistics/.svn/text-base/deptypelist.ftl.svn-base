...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body>
		<div class="iner_title_name"><strong>统计分析>>禁毒机构统计>>按机构性质统计</strong></div>
		<#include "../../common/msg_panel.ftl" />
		<#include "../../common/export/expform.ftl" />
		<div class="iner_title_sub_03">
			<form id="staffDtoForm" action="${contextPath}/report/deptstatistics/searchdeptype.action" method="post">
		...
		地区：<select class="serch" id="province" onchange="cascade(1, '${user.areaCode}', '${user.areaCode}');">
			 ...
			 &nbsp;&nbsp;
		      机构立功表彰情况：<select class="form_select" name="deptdto.deptobserving" id="select2">
		      ...
		      </select>机构职级：<select class="form_select" name="deptdto.deptdict" id="select1">
			   	<option value="0">==请选择==</option>
		        <#list dictRanks as dict>
			        <option value=${dict.pid} <@s.if test="${dict.pid}==deptdto.deptdict">selected</@s.if>>${dict.vcName!""}</option>
			     </#list>
		      </select>
		      &nbsp;&nbsp;&nbsp;&nbsp;<input type="button" onclick="formSubmit();" size="12" value="　查询　" /> 
			  <input type="button" size="12" value="　导出　" onclick="downXLS('tab1','按机构性质统计')" />
			</form>
		</div>
				...
				<#list deptstatistics as depts>
					<tr align="center">
						<#list depts as dept>
							...
							<th width='${100/depts?size}%'>${dept!0}</th>
							<#else><td>${dept!0}</td>
							</#if>
						</#list>
					</tr>
