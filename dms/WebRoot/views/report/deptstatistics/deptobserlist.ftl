...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body>
		<div class="iner_title_name"><strong>统计分析>>禁毒机构统计>>部门立功表彰次数统计</strong></div>
		<#include "../../common/msg_panel.ftl" />
		<#include "../../common/export/expform.ftl" />
		<div class="iner_title_sub_03">
			<form id="staffDtoForm" action="${contextPath}/report/deptstatistics/searchobsertype.action" method="post">
		...
		地区：<select class="serch" id="province" onchange="cascade(1, '${user.areaCode}', '${user.areaCode}');">
			 ...
			 &nbsp;&nbsp;
		      机构性质：<select class="form_select" name="deptdto.deptype" id="select1">
			   ...
			   <input type="button" size="12" value="　导出　" onclick="downXLS('tab1','按机构立功表彰统计')" />
			</form>
		</div>
