...
<html xmlns="http://www.w3.org/1999/xhtml">	
	<head>
		...
		<script type="text/javascript">
        	function formSubmit() {
				...
				$.post(url, {'pmBscObserType.vcName': vcName, 'vctype':${vctype}}, function(jsonObj) {
					if(jsonObj.timeout) {
	...
	<body>
		<div class="iner_title_name"><strong>系统管理>>代码维护>><@s.if test="vctype == 1">机构立功表彰新增</@s.if>
	<@s.if test="vctype != 1">人员立功表彰新增</@s.if></strong></div>
		<div class="iner_title_sub_03">
			<a href="javascript:doCancel();" class="return" >返回</a>
		</div>
		<@s.set name="formUrl" value="'/baseinfo/obsertype/addDepType.action?vctype=${vctype}'" />
		<#include "form.ftl" />
	</body>
</html>		
